...
def main():
    ...
    full_path_traveled = []  # 紀錄當前點位走過的完整路徑
    ...
    while np.linalg.norm(current_position - goal) > 0.1:
        ...
        if line_intersects_any_obstacle(current_position, next_position, obstacles, safety_distance):
            ...
            visualize_grid(ax, start, goal, obstacles, circle_radius=circle_radius, positions=current_position, original_path=initial_path, path = full_path_traveled, new_points=new_path)
            # plt.pause(0.5) # 暫停觀看偏移路徑

            # 使用牛頓法進行路徑最佳化
            optimized_path, iter_num = newton_method(new_path, points_on_obstacle, W, closest_obstacle, safety_distance)
            # 確認新路徑是否避開障礙物
            ...
            for i in range(len(X_opt_final) - 1):
                if line_intersects_any_obstacle(X_opt_final[i], X_opt_final[i+1], obstacles, safety_distance):
                    ...
                    optimized_path, iter_num = newton_method(optimized_path, points_on_obstacle, W, closest_obstacle, safety_distance)
            # 更新 origin_path
            ...
            visualize_grid(ax, start, goal, obstacles, circle_radius=circle_radius, positions=current_position, original_path=initial_path, path = full_path_traveled, new_points=X_opt_final)
            # plt.pause(0.5) # 暫停觀看最佳化路徑
            # 檢查是否到達終點
...
# 計算總時間
end_time = time.time()  # Record the time point when the program execution ends
total_time = end_time - start_time
# 顯示總距離和時間結果
print("Total path length: ", path_length, "Total execution time: ", total_time, "seconds")
